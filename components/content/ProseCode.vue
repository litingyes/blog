...
<template>
  <div ref="proseCodeRef" class="relative border-1 border-slate-200 rounded-lg border-solid transition hover:shadow-lg">
    <div class="absolute right-4 top-2 text-lg text-slate-500">
      <span v-show="!isHover" class="">{{ props.language }}</span>
      <span v-show="isHover && !copied" class="i-fluent:copy-16-regular cursor-pointer hover:text-slate-900" @click="copyCode" />
